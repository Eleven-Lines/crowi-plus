...
class PassportService {
  ...
  setupMikanStrategy() {
    ...
    const isMikanEnabled = Config.isEnabledPassportMikan(config);
    const apiUrl = config.crowi['security:passport-mikan:apiUrl'];

    if (!isMikanEnabled || !apiUrl) {
      return;
    }

    passport.use(new CookieStrategy(
      {
        cookieName: config.crowi['security:passport-mikan:cookieName'],
      },
      (token, done) => { process.nextTick(() => {
        request.post({
