module.exports = function(crowi, app) {
  ...
  actions.user.authorize = function(req, res) {
    var id = req.params.id;
    User.findById(id, function(err, userData) {
      userData.authorize(function(err, userData) {
        if (err === null) {
          req.flash('successMessage', userData.name + 'さんのアカウントからの編集を許可しました。');
        } else {
          req.flash('errorMessage', '更新に失敗しました。');
          debug(err, userData);
        }
  ...
  actions.user.unauthorize = function(req, res) {
    var id = req.params.id;
    User.findById(id, function(err, userData) {
      userData.unauthorize(function(err, userData) {
        if (err === null) {
          req.flash('successMessage', userData.name + 'さんのアカウントからの編集を禁止しました。');
        } else {
          req.flash('errorMessage', '更新に失敗しました。');
          debug(err, userData);
        }
  ...
  actions.api.securityPassportMikanSetting = function(req, res) {
    var form = req.form.settingForm;
    ...
    return saveSettingAsync(form)
      .then(() => {
        ...
        // setup strategy
        if (Config.isEnabledPassportMikan(config)) {
