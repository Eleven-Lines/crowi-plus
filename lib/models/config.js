module.exports = function(crowi) {
  ...
  function getDefaultCrowiConfigs()
  {
    return {
      ...
      'conoha:username'           : '',
      'conoha:password'           : '',
      'conoha:identityServiceUrl' : 'https://identity.tyo1.conoha.io',
      'conoha:region'             : 'tyo1',
      'conoha:tenantId'           : '',
      'conoha:container'          : 'crowi',
  ...
  configSchema.statics.isEnabledPassportMikan = function(config)
  {
    const key = 'security:passport-mikan:isEnabled';
    return getValueForCrowiNS(config, key);
  };
  ...
  configSchema.statics.isUploadable = function(config)
  {
    ...
    if (method == 'conoha' && (
        !config.crowi['conoha:username'] ||
        !config.crowi['conoha:password'] ||
        !config.crowi['conoha:identityServiceUrl'] ||
        !config.crowi['conoha:region'] ||
        !config.crowi['conoha:tenantId'] ||
        !config.crowi['conoha:container'])) {
      return false;
    }
