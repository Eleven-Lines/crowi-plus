module.exports = function(crowi) {
  ...
  pageSchema.methods.isGrantedFor = function(userData) {
    ...
    if (userData.authorized == 1 && this.grant == GRANT_AUTHORIZED) {
      return true;
    }
  ...
  pageSchema.statics.getGrantLabels = function() {
    ...
    grantLabels[GRANT_AUTHORIZED] = 'Authorized user only'; // 編集権限のあるユーザーのみ
  ...
  pageSchema.statics.findListByCreator = function(user, option, currentUser) {
    ...
    var conditions = {
      $and: [
        {creator: user._id, redirectTo: null},
        {$or: [
          {status: null},
    ...
    if (!user.equals(currentUser._id)) {
      let grant_queries = [{grant: GRANT_PUBLIC}]
      if (currentUser.authorized) {
        grant_queries.push({grant: GRANT_AUTHORIZED});
      }
      conditions['$and'].push({$or: grant_queries});
    }
  ...
  pageSchema.statics.GRANT_AUTHORIZED = GRANT_AUTHORIZED;
