module.exports = function(crowi) {
  ...
  userSchema = new mongoose.Schema({
    ...
    authorized: { type: Boolean, default: 1, index: true  }
  });
  ...
  userSchema.methods.authorize = function(callback) {
    debug('Authorized', this);
    this.authorized = 1;
    this.save(function(err, userData) {
      return callback(err, userData);
    });
  ...
  userSchema.methods.unauthorize = function(callback) {
    if (this.admin) {
      debug('Admin cannnot be unauthorized');
    }
    else {
      debug('Unauthorized', this);
      this.authorized = 0;
    }
    this.save(function(err, userData) {
      return callback(err, userData);
  ...
  userSchema.statics.findRoms = function(callback) {
    var User = this;
    this.find({readonly: true})
      .exec(function(err, roms) {
        debug('ROMs: ', roms);
        callback(err, roms);
      });
  };
